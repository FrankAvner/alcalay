...
class GoogleDriveWindow(QDialog):
    ...
    def run_keyword_filter(self):
        """Run the existing Drive keyword filter executable."""
        self.log("Google Drive keyword filter requested.")
...
class RepositoryOperationsPage(QWidget):
    """Central page for the repository operations that must remain visible."""
    ...
    def __init__(self, parent=None):
        ...
        title = QLabel("פעולות מאגרים")
        title.setStyleSheet("font-size: 28px; font-weight: bold; padding: 8px;")
        layout.addWidget(title)

        info = QLabel(
            "כאן מרוכזות פעולות הסנכרון, האינדוקס, ייבוא המיילים והקטלוג."
        )
        ...
        sync_title = QLabel("סנכרון בין מאגרים")
        sync_title.setStyleSheet("font-size: 20px; font-weight: bold;")
        sync_layout.addWidget(sync_title)

        local_to_drive = QPushButton("⬆  סנכרון מקומי → Google Drive")
        local_to_drive.setMinimumHeight(52)
        local_to_drive.clicked.connect(
            lambda: self.open_sync(DIRECTION_LOCAL_TO_DRIVE)
        )
        sync_layout.addWidget(local_to_drive)

        drive_to_local = QPushButton("⬇  סנכרון Google Drive → מקומי")
        drive_to_local.setMinimumHeight(52)
        drive_to_local.clicked.connect(
            lambda: self.open_sync(DIRECTION_DRIVE_TO_LOCAL)
        )
        sync_layout.addWidget(drive_to_local)
        layout.addWidget(sync_frame)

        index_frame = QFrame()
        index_layout = QVBoxLayout(index_frame)
        index_title = QLabel("אינדוקס")
        index_title.setStyleSheet("font-size: 20px; font-weight: bold;")
        index_layout.addWidget(index_title)

        index_button = QPushButton("🗂  פתיחת אינדוקס מקומי")
        index_button.setMinimumHeight(52)
        ...
        import_title = QLabel("ייבוא")
        import_title.setStyleSheet("font-size: 20px; font-weight: bold;")
        import_layout.addWidget(import_title)

        import_button = QPushButton("✉  ייבוא מיילים מ-Gmail")
        ...
        catalog_title = QLabel("קטלוג")
        catalog_title.setStyleSheet("font-size: 20px; font-weight: bold;")
        catalog_layout.addWidget(catalog_title)

        catalog_button = QPushButton("📚  ניהול קטלוג / קטגוריות")
        catalog_button.setMinimumHeight(52)
        catalog_button.clicked.connect(self.open_catalog)
        catalog_layout.addWidget(catalog_button)
        layout.addWidget(catalog_frame)

        layout.addStretch()

    def open_sync(self, direction):
        try:
            window = DriveSyncWindow(direction, self)
            self.sync_windows.append(window)
            window.show()
            window.raise_()
            window.activateWindow()
        except Exception as exc:
    ...
    def open_index(self):
        try:
            ...
            QMessageBox.critical(
                self,
                "אינדוקס",
                f"שגיאה בפתיחת חלון האינדוקס:\n{exc}",
            )

    def open_import(self):
        try:
            if self.import_window is None:
                self.import_window = GmailImportWindow(self)
            ...
            QMessageBox.critical(
                self,
                "ייבוא Gmail",
                f"שגיאה בפתיחת חלון הייבוא:\n{exc}",
            )

    def open_catalog(self):
        try:
            if self.category_window is None:
                self.category_window = PlaceholderPage("קטלוג / ניהול קטגוריות", self)
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        for item_text in menu_items:
            ...
            if item_text in ("חיפוש", "קטגוריות", "מערכת"):
        ...
        self.add_page(
            "drive_keyword_filter",
            GoogleDriveWindow(self),
        )
        self.add_page(
            "ייבוא מיילים מ-Gmail",
            GmailImportWindow(self),
        )
        self.gmail_index_window = GmailIndexWindow(self)
        self.add_page(
            "עיבוד EML / PARSE",
    ...
    def change_page(
        ...
        if name == "סנכרון בין מאגרים":
            operations = self.pages.get("סנכרון בין מאגרים")
            if operations is not None:
                self.stack.setCurrentWidget(operations)
                return

        if name == "אינדוקס":
            operations = self.pages.get("סנכרון בין מאגרים")
            if operations is not None:
                operations.open_index()
                return

        if name == "עיבוד EML / PARSE":
            parser_window = self.pages.get("עיבוד EML / PARSE")
            if parser_window is not None:
                self.stack.setCurrentWidget(parser_window)
                parser_window.show()
                parser_window.raise_()
                parser_window.activateWindow()
                return

        if name == "ייבוא מיילים מ-Gmail":
            operations = self.pages.get("סנכרון בין מאגרים")
            if operations is not None:
                operations.open_import()
                return

        if name == "קטלוג":
            operations = self.pages.get("סנכרון בין מאגרים")
            if operations is not None:
                operations.open_catalog()
                return

        if name == "drive_keyword_filter":
            drive_page = self.pages.get("drive_keyword_filter")
            if drive_page is not None:
                self.stack.setCurrentWidget(drive_page)
                drive_page.run_keyword_filter()
                return
